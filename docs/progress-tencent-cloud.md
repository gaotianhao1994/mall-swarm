...
> **系统**: Ubuntu 22.04 LTS | **角色**: 网关/入口服务器（Nacos + Gateway + Nginx）
> **最后更新**: 2026-06-11 (架构调整日)

---

## 架构调整说明

> **2026-06-11**: 因 CPU 负载达 90%，将 MySQL/Redis/Admin/Auth 迁移至新数据服务器 `106.53.132.192`，
> 本服务器调整为仅运行轻量级网关层服务。
...
| 项目 | 调整前 | 调整后 |
|------|--------|--------|
| 部署服务 | MySQL+Redis+Nacos+Gateway+Admin+Auth (6个) | Nacos+Gateway+Nginx (3个) |
| 预估内存占用 | ~3.5GB / 4GB | ~1.2GB / 4GB |
| 预估CPU负载 | ~90% | ~30% |

### 待执行迁移操作

- [ ] 停止当前所有容器：`docker compose down`
- [ ] 将 MySQL 数据迁移到新数据服务器
- [ ] 仅保留 Nacos + Gateway + Nginx 编排文件
- [ ] 重新启动精简后的服务
- [ ] 安全组端口调整：移除 3306/6379/8080/8401，保留 22/80/443/8848/8201
...
|------|------|------|
| **阶段0** | 环境准备 | ✅ **已完成** |
| **阶段1 Part A** | 网关层（Nacos + Gateway + Nginx） | 🔧 **待重新规划（需拆分编排文件）** |
| 阶段1 Part B | 基础数据层（MySQL + Redis + Admin + Auth） | ➡️ 已迁移至数据服务器 106.53.132.192 |
| 阶段2~5 | 后续阶段 | ☐ 待开始 |
...
### SSH 连接信息
```bash
ssh tengxun-server        # 或 ssh root@106.53.106.41
```
